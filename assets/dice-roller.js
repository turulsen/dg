    ...
    function ensureFirebaseApi(cb) {
        ...
        const afterCore = () => {
            if (!window.firebase.apps.length) {
                ...
                // Brave (and some ad-blocker extensions) silently
                // blocks Firestore's default streaming transport
                // (WebChannel) -- it looks like a long-lived tracking
                // connection -- which leaves every onSnapshot()
                // listener permanently stuck with zero data and no
                // error at all. Falls back to plain HTTP long-polling,
                // which isn't blocked. See a-cell.html's own copy of
                // this comment for the full report that traced this
