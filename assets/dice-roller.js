    ...
    function ensureFirebaseApi(cb, onerror) {
        ...
        const afterCore = () => {
            if (!window.firebase.apps.length) {
                ...
                // with zero data, and each failed reconnect attempt as an
                // uncaught exception inside this cross-origin script (the
                // repeating "Script error." flood the 2026-09-11 live
                // report showed). Auto-detect means trying the streaming
                // transport first and only falling back after it fails --
                // forcing long-polling from the start skips that failing
                // dance entirely on exactly the networks that need it, at
                // the cost of slightly higher latency everywhere else.
