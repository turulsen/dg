...
## Legacy Google Drive-hosted tracks went silently unplayable, and the Handler's own UI hid the failure

Live report: "Abyss plays. The old ones don't, and the one with the
shitty title doesn't [play] either." Checked directly against the live
Firestore data and the real Drive API response rather than guessing --
two separate, real causes, one data and one code.

**Cause 1 (data, not code): an expired Google Drive API key.**
`tracks/{trackId}` docs for four legacy, pre-Firebase-migration tracks
(Combat, The Conspiracy, The Schism, Phantom) each store a `url` field
built once, at upload/backfill time, from `driveDirectAudioUrl(fileId)`
(`backend/Code.gs`) -- `https://www.googleapis.com/drive/v3/files/
{id}?alt=media&key={GOOGLE_DRIVE_API_KEY}`. Fetching that exact URL
directly returns HTTP 400: `"API key expired. Please renew the API
key."` A fifth legacy track (Piano), uploaded under a *different* Drive
API key, still returns HTTP 200 with real audio bytes -- confirming
this is a per-key expiry, not a Drive-wide or account-wide problem.
Every Storage-hosted track (Phase 4+, including the one with the
autogenerated filename-as-title -- confirmed still fetches 200/audio
directly) is unaffected: it self-heals its URL from `tracks/{id}.mp3`
on every read (`canonicalTrackUrl_()`), but a legacy Drive track's
Firestore `url` is a frozen snapshot with no equivalent self-heal --
nothing re-derives it after the one-time backfill, so an expired key
breaks it permanently with no code-level recovery. Not fixed here: this
needs either a fresh `GOOGLE_DRIVE_API_KEY` (Script Properties) plus a
re-run of `runBackfillTracksToFirestoreNow()`, or -- the more durable
fix, matching this app's own direction of killing the Drive/Apps
Script dependency entirely -- re-uploading these tracks through the
Storage-based Track Library upload flow and deleting the old Drive-
backed entries.

**Cause 2 (real code bug, fixed): a genuine failure was actively hidden
from the Handler.** The preview `<audio>` element already has an
`'error'` listener (added for an earlier, similarly-shaped "frozen
playback" bug) that correctly sets `#music-status` to "Playback failed
-- this track isn't reachable right now." on exactly this failure. But
`startNowPlayingListener_()`'s `onSnapshot` callback unconditionally
overwrote that same element with "Currently broadcasting to CH N:
{title}" on *every* write to that channel's `radio/{ch}` doc --
including ones with nothing to do with the main track at all (an
ambient layer toggle, a stinger fire, a volume drag). That doc gets
touched constantly during real play, so the true error was visible for
at most a moment before being silently re-hidden behind a false "all
clear" -- exactly matching the live report: a confident "Currently
broadcasting" status with a live Pause icon, scrubber frozen at 0:00,
and no visible error anywhere. This is why Combat/Conspiracy/Schism/
Phantom looked like a totally unexplained hang rather than a named
failure. Fixed with a `previewAudioBroken` flag: set when the preview
element's `'error'` listener fires, cleared whenever a genuinely new
`track_url` loads, and checked by the `onSnapshot` callback so a
still-broken *current* track's status is left alone instead of being
overwritten back to "Currently broadcasting" on the next unrelated
write.

New regression test `test_acell_music_broadcast_error_not_clobbered`
confirms both halves: the error surfaces at all, and survives an
unrelated write to the same doc. Verified failing without the fix
(reproduces "Currently broadcasting to CH 1: Old Broadcast" over a
confirmed-unreachable track, matching the live report exactly) and
passing with it. Full regression: `test_acell_gate`, `test_acell_play`,
`test_acell_music`, `test_acell_music_broadcast_error_not_clobbered`,
`test_acell_soundboard`, `test_acell_music_backend_not_deployed`,
`test_table_radio_widget`, `test_table_radio_pause_and_loop`,
