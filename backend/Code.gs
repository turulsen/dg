...
//   this fix existed.
// + v84 -- new log_client_error action (getOrCreateClientErrorsSheet()/
//   logClientError(), 14-day retention): backs the new page-wide
//   assets/js-error-banner.js telemetry added after the 2026-09-10
//   incident, whose actual root cause was never confirmed for lack of
//   any error visibility beyond one on-screen banner on a-cell.html
//   alone. Deliberately unauthenticated (same posture as
//   mark_evidence_seen) and independent of every other action in this
//   file, so it keeps working even if something else here is what's
//   broken. NOTE: "v83" is skipped on purpose -- an earlier, unrelated
//   backend change from the same incident's 141-commit branch called
//   itself v83 but was never actually pasted into this project (that
//   whole branch was rolled back after the incident); reusing v83 here
...
//   at all.
// + v86 -- updateCellMembers() (the only place Cell membership is ever
...
// Real recorded SFX (assets/ambient/*.mp3, assets/stingers/*.mp3),
// replacing an earlier procedurally-synthesized attempt that was built,
// tested, and deliberately abandoned for poor audio quality -- see
// design-graveyard/table-radio-audio-soundscape's RETROSPECTIVE.md.
// Both fields are dual-patched straight onto the same radio/{channel}
// Firestore document Now Playing already lives on, so the existing
// onSnapshot listener in table-radio.js picks up every toggle/trigger
// instantly -- no separate listener, collection, or poll loop needed.
// ════════════════════════════════════════════════════════════════
const AMBIENT_LAYER_IDS = ['alien-lunch', 'any-human-heart', 'haunted', 'howling-city-bells', 'music-box-morning', 'drone-dead', 'drone-low'];
const STINGER_IDS = [
...
// after the first survives instead of silently clobbering it, letting
// players' clients actually overlap/layer them via separate <audio>
