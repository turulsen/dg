...
- **Fully Firestore-backed (reads and writes)**: Table Radio's ambient-layer toggles, stinger fires, AND the Active Sounds panel's own per-instance transport (pause/resume/seek/loop/stop an already-active loop or stinger), the main track transport too now (set/pause/resume/restart/seek/loop/broadcast-wide mix — no Apps Script action, no confirmation poll, a live `radio/{channel}` listener is the truth for all of it), the cross-page Dice Roller history and Live Rolls, the Track Library (upload/list/delete go straight to Firestore + Firebase Storage, zero Apps Script involvement), the persistent DEX Initiative Tracker (Agent Hub + A-Cell Play/Cells).
- **Firestore for reads, Apps Script still the write path of record**: Player Notes, the Evidence Locker, A-Cell's Play tab roster, A-Cell's Cells tab roster (writes — `create_cell`/`update_cell_members`/`delete_cell` — still go through Apps Script, which dual-writes to Firestore server-side), Table Radio's Cue List (`get_playlist`/`save_playlist`) and Cue For Cell (`set_cell_channel`) — the clearest remaining "partial migration" surfaces now.
