...
**Handler authentication is now unified on a single Firebase sign-in.** Typing the password into A-Cell's gate (right after Clearance) calls the `handlerLogin` Cloud Function directly and signs into Firebase; that ID token (carrying a `handler:true` custom claim) is the only Handler credential anywhere — Firestore's rules check it via `isHandler()`, and every remaining Handler-gated Apps Script action verifies the same token server-side via `verifyHandlerIdToken_()` (Identity Toolkit's `accounts:lookup` REST endpoint, since Apps Script can't run the Admin SDK). The old separate Apps Script session (`handler_login` POST action, opaque `dg_acell_session` token, `requireHandlerSession_()`/`HANDLER_SESSION_TTL_SECONDS`) is gone entirely — Code.gs never sees the raw password at all now. See `BUGFIXES.md`'s "Handler auth unification" entry for the full before/after.
