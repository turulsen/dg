...
- **Fully Firestore-backed (reads and writes)**: Table Radio's ambient-layer toggles, stinger fires, AND the Active Sounds panel's own per-instance transport (pause/resume/seek/loop/stop an already-active loop or stinger — no Apps Script action, no confirmation poll, the transaction's own resolved state is the truth), the cross-page Dice Roller history and Live Rolls, the Track Library (upload/list/delete go straight to Firestore + Firebase Storage, zero Apps Script involvement), the persistent DEX Initiative Tracker (Agent Hub + A-Cell Play/Cells).
- **Firestore for reads, Apps Script still the write path of record**: Player Notes, the Evidence Locker, A-Cell's Play tab roster, A-Cell's Cells tab roster (writes — `create_cell`/`update_cell_members`/`delete_cell` — still go through Apps Script, which dual-writes to Firestore server-side), Table Radio's MAIN TRACK transport (play/pause/seek/loop/volume) — the clearest remaining "partial migration" surface now, still reading its confirmation back via legacy JSONP instead of a live Firestore doc.
