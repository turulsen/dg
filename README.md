...
- **Fully Firestore-backed (reads and writes)**: Table Radio's Now Playing + the ambient/stinger soundboard, the cross-page Dice Roller history and Live Rolls, the Track Library (upload/list/delete go straight to Firestore + Firebase Storage, zero Apps Script involvement), the persistent DEX Initiative Tracker (Agent Hub + A-Cell Play/Cells).
- **Firestore for reads, Apps Script still the write path of record**: Player Notes, the Evidence Locker, A-Cell's Play tab roster, A-Cell's Cells tab roster (writes — `create_cell`/`update_cell_members`/`delete_cell` — still go through Apps Script, which dual-writes to Firestore server-side).
...
**Handler authentication is two separate systems, not one**, both driven by the same single password the Handler types into A-Cell's gate: an Apps Script session (`handler_login` action, cached as `dg_acell_session`) that every remaining Sheet-write action needs, and a Firebase custom-token sign-in (`ensureHandlerSignedIn()`, via the `handlerLogin` *Cloud Function* — a different backend from the Apps Script action of the same-sounding name) that anything gated by Firestore's `isHandler()` rule needs (Evidence writes, Track Library, Live Rolls). A prior pass consolidated multiple in-page *copies* of the Firebase-Auth flow into one (`window.__dgHandlerAuth`), but the two backends themselves are still both live and independent — fully unifying them would mean retiring the Apps Script session path in favor of Firestore writes gated by `isHandler()` for whatever Sheet-write actions remain, which hasn't been done.
